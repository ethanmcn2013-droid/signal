...
/**
 * Signal hero — "The Brief" (distillation: a packed pile of noise resolves into
 * the front page).
 *
 ...
 * line. Everything hangs to one left optical edge; the only colour at rest is
 * the indigo editor's pencil in the lead gutter.
 *
 * FIRST FRAME: the masthead rule + folio + dateline are crisp from t=0 (the
 * paper's constant identity). Below the rule, the page body is a dense mosaic of
 * roughly forty tiny grey note chips — everyday work noise, a packed noticeboard.
 * No colour, no blur. A very slow idle drift keeps it alive.
 *
 ...
 *   E REST    — the clean front page. The SET ASIDE index and ledger fade in last.
 *
 ...
 * keyframes kicked off by a mount-triggered, self-starting rAF; every rAF and
 * timer is tracked and cancelled on unmount. Under prefers-reduced-motion the
 * whole intro short-circuits to the settled front page — no note field, no
 * motion, no delay.
 */
...
// Reduced-motion is read through an external store so no setState is called
// synchronously inside the effect (which would trigger cascading renders). The
// server snapshot is "not reduced" — the CSS reduced-motion media query is the
// real first-frame safety net, so there is no flash before hydration settles.
...
export function SignalHeroSig5() {
  ...
  // "started" flips after first paint to run the choreography from the packed
  // first frame. "restReached" strips will-change and stops the idle drift.
  const [started, setStarted] = useState(false);
  const [restReached, setRestReached] = useState(false);

  // Tracked so cleanup can cancel every scheduled rAF and timer on unmount.
  const rafsRef = useRef<number[]>([]);
  const timersRef = useRef<ReturnType<typeof setTimeout>[]>([]);

  useEffect(() => {
    // Reduced motion: the settled front page is rendered purely from derived
    // state below. No field, no rAF, no timers, no delay.
    if (reduced) return;

    const rafs = rafsRef.current;
    const timers = timersRef.current;

    // Kick the choreography off after first paint so the keyframes start clean
    // from the packed first frame. Mount-triggered, self-starting, no observer.
  ...
  const mode: Mode = reduced ? "static" : started ? "animate" : "pre";
  const atRest = reduced || restReached;
  ...
  return (
    <section className={sectionClass} style={ROOT_VARS}>
      ...
      <div className={`${PREFIX}-page`}>
        {/* Decorative note field — the packed pile, absolute below the rule. */}
...
const ROOT_VARS: CSSVars = {
  ...
  "--sig5-ink": "#111111",
  "--sig5-ink-soft": "#3f3f46",
  "--sig5-ink-faint": "#71717a",
  "--sig5-paper": "#ffffff",
  ...
  "--sig5-hairline": "rgba(17,17,17,.06)",
...
@media (prefers-reduced-motion: reduce){
  ...
  .${PREFIX}-kicker,
  .${PREFIX}-aside,
  .${PREFIX}-ledger{opacity:1 !important;}
  .${PREFIX}-band-rule,
