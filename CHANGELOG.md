...
## 2026-07-05 · A·9 · cuts · the daily brief is two blocks everywhere, not two on web and four in the shop window

**The morning read is now the same shape on every surface: Needs attention, then Quiet risks — two blocks.** On 2026-06-09 (A·3) the web brief cut "Moving well" and "Suggested focus" from the daily read, but the paid daily email, the homepage demo, and the wedding-planning sample were never brought along — they kept showing four. A visitor watched a four-block demo, read a four-block sample, signed in, and received a two-block brief. That gap is closed. The daily email (HTML and plain-text) now renders the two attention blocks; "Moving well" (celebration — its `just-shipped` trigger is the lowest-weighted of six, the engine had already ranked it as not-the-signal) and "Suggested focus" (a sorted re-projection of items already shown above) are gated to the **weekly** cadence, where a ten-minute Friday review can carry them without crowding the morning. The homepage demo filters to the two daily blocks (the fuller data stays in `domains.ts` for a future weekly demo); the sample page shows the two a reader actually receives. The grace-note sign-off no longer points at a "focus block" the daily read doesn't render — it steers to the top of the list instead. `PRODUCT.md §4` is amended from "Always four" to the daily-two / weekly-four split, with the reason recorded in the document per its own §12 (fix the contract first, then the surfaces). The engine still computes all four blocks on every build; the render layer decides which surface shows which, keyed on cadence. Typecheck clean, 169 tests pass (daily two-block and weekly four-block are now both asserted), DS drift gate clean.
