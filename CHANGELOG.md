...
## 2026-07-13 · A·12 · holds · the briefing keeps the top line

**Signal can now grow beneath the morning brief without turning into the dashboard it replaced: Briefing stays at zero to three things, while Overview, Trends, and Evidence answer the next question only when the reader asks it.** The product contract now names the boundary plainly. Overview arrives useful before setup. Trends takes one metric at a time and refuses to invent history. Evidence carries the deterministic rule, comparison, coverage, and real source records behind every claim. Customization stops at hide, pin, reorder, and restore.

The implementation now exists behind one production-off flag: integrated Briefing, Overview, Trends, and Evidence routes; deterministic rules and source adapters; and a forward-only aggregate snapshot job scheduled daily at 02:30 UTC. The rule engine remains the authority; optional narrative wording can never calculate the numbers, select the concern, or outrun the evidence. This entry still records a **holds** release, not a production launch. It becomes a shipped entry only after tenant isolation, source links, keyboard behavior, live staging data, the authenticated snapshot run, and the production build are verified.
